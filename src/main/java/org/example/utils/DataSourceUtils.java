...
public class DataSourceUtils {
    ...
    public static Stream<String> loadDataFile(String fileName) throws IOException {
        return Files.lines(Paths.get(System.getProperty("user.dir") +  fileName)).skip(1);
    }

    public static Item lineAsItem(String line){
        String[] tokens = line.split(",");
        long id = Integer.parseInt(tokens[0]);
        long createAtEpoch = Long.parseLong(tokens[2]);
        double price = Double.parseDouble(tokens[7]);
        return new Item(id, createAtEpoch, tokens[3], tokens[4], tokens[5], tokens[6], price);
    }
